...
**Conditional ship** — merge to `main` after addressing the **major**
...
**README says** (lines 101–102): _"An action runs when any watched pattern
...
unnecessarily when no matching files are in scope — opposite of documented and
...
**Impact:** Rare but high severity — silent loss of working-tree state after
hook.
...
- `ActionFailedErr` on `main` had no index/total; revamp adds `2/5` style
  messages — good improvement, now tested in `util_err_test.gleam`.
- `help` no longer accepts raw `--help` in `cactus.gleam` case — correctly
...
`False` — correct for Windows users.
...
- `files_scope` inheritance from hook → action works via `hook_default`
  parameter.
- `on_failure = "continue"` correctly collects first error and fails at end —
  **untested at runtime**.
- Pre-commit `run_with_stash` flow is logically sound when stash/pop succeed.
- `do_run` uses `action.cwd` for both file scope queries and command execution —
